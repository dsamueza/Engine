...
namespace Mardis.Engine.DataObject.MardisCore
{
    public class TaskCampaignDao : ADao
    {
        ...
        public MyTaskViewModel GetForProfile(Guid idTask, Guid idAccount)
        {
            ...
            var tasks = Context.Query<MyTaskViewModel>($@"EXEC dbo.sp_Campaign_Information_new_code @idtask='{idTask}'").FirstOrDefault();
#if DEBUG  
            myWatch.Stop();
        ...
        public MyTaskViewModel UpdatestoreAuditTBHecho(Guid idTask, Guid idAccount)
        {
            ...
            var task = Context.Query<MyTaskViewModel>($@"EXEC dbo.sp_proc_actualizar_hechos_store_audit @idtask='{idTask}'").FirstOrDefault();
        ...
        public async Task<int> AccessTheWebAsync(Guid idTask, Guid idAccount)
        {
            var task = Context.Query<MyTaskViewModel>($@"EXEC dbo.sp_proc_actualizar_hechos_store_audit @idtask='{idTask}'").FirstOrDefault();
        ...
        public void ValidateTask(Guid idTask, Guid idStatus, Guid idAccount, Guid idUser)
        {
            ...
            task.UserValidator = idUser;
            task.DateValidation = DateTime.Now.AddHours(-5);
            InsertOrUpdate(task);
        }
