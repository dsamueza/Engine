...
namespace Mardis.Engine.DataObject.MardisCore
{
    public class BranchDao : ADao
    {
        ...
        public bool IsUseCode(string code)
        {
            try
            {
                var Status = Context.StoreAuditCodes.Where(x => x.Code.Equals(code) && x.idbrach ==null).Count()>0? true:false;
        ...
        public bool SaveNewCodeBranch(string code,Guid idbranch,Guid user,Guid idtask)
        {
            ...
            //               
            {
                try
                {
                    var _modelBranch = Context.Branches.Where(x => x.Id.Equals(idbranch)).First();
                    _modelBranch.Code = code;
                    _modelBranch.StatusRegister = "A";
                    Context.Branches.Add(_modelBranch);
                    Context.Update(_modelBranch);
                    Context.SaveChanges();


                    var _modelCode = Context.StoreAuditCodes.Where(x => x.Code.Equals(code)).First();
                    _modelCode.idbrach = idbranch;
                    _modelCode.assignDate = DateTime.Now.AddHours(-5);
                    _modelCode.Users = user;
                        Context.StoreAuditCodes.Add(_modelCode);
                        Context.Update(_modelCode);
                        Context.SaveChanges();

                   transaction.Commit();

                    var task = Context.Query<ViewProcedureModel>($@"EXEC dbo.sp_change_code_external_base @idtask='{idtask}',@code='{code}'").FirstOrDefault();
                    return true;
                }
                catch (Exception e)
                {
